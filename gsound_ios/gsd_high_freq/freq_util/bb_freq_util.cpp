...
static int freq_init_is_high = true;
...
int freq_to_num(unsigned int f, int *n) {
    ...
    freq_init();
  //  printf("\n f[0] = %f,f[31] = %f\n",frequencies[0],frequencies[31]);
  //  printf("\n f = %u,min = %f,max = %f\n",f,frequencies[0]-BB_THRESHOLD,frequencies[31]+BB_THRESHOLD);
    if (n != NULL &&f >= frequencies[0]-BB_THRESHOLD&&f <= frequencies[31]+BB_THRESHOLD) {
        
        unsigned int i;
        
        for (i=0; i<32; i++) {
            
            if (std::abs(frequencies[i] - f) <= BB_THRESHOLD) {
